...
def test_get_endpoint_address_should_raise_value_error_if_no_endpoint(
    api_service: DirectoryApiService, mock_fhir_api: MagicMock
) -> None:
    ...
    with pytest.raises(HTTPException, match="Endpoint bar not found in included resources"):
        api_service.fetch_directories()

def test_get_endpoint_address_should_raise_value_error_if_multiple_endpoints(
    api_service: DirectoryApiService, mock_fhir_api: MagicMock
) -> None:
    ...
    with pytest.raises(HTTPException, match="multiple endpoints"):
        api_service.fetch_directories()

def test_get_endpoint_address_should_raise_value_error_if_no_matching_endpoint(
    api_service: DirectoryApiService, mock_fhir_api: MagicMock
) -> None:
    entries = __mock_bundle_entry()
    ...
    with pytest.raises(HTTPException, match="not found in included resources"):
        api_service.fetch_directories()

def test_get_endpoint_address_should_raise_value_error_if_address_missing(
    api_service: DirectoryApiService, mock_fhir_api: MagicMock
) -> None:
    entries = __mock_bundle_entry()
    try:
    ...
    with pytest.raises(HTTPException, match="No endpoint address for Endpoint"):
        api_service.fetch_directories()
