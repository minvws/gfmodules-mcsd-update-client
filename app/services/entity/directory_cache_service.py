...
class DirectoryCacheService:
    ...
    def get_deleted_directories(self) -> list[DirectoryDto]:
        ...
        with self.__database.get_db_session() as session:
            ...
            return [
                self._hydrate_to_directory_dto(directory)
                for directory in deleted_directories
            ]

    def get_directory_cache(
        self, directory_id: str, with_deleted: bool = False
    ) -> DirectoryDto | None:
        """
    ...
    def get_all_directories_caches(
        self, with_deleted: bool = False, include_ignored: bool = False
    ) -> list[DirectoryDto]:
        """
        ...
        with self.__database.get_db_session() as session:
            repository = session.get_repository(DirectoryCacheRepository)
            return [
                self._hydrate_to_directory_dto(cache)
                for cache in repository.get_all(
                    with_deleted=with_deleted, include_ignored=include_ignored
                )
            ]

    def get_all_directories_caches_include_ignored_ids(
        self, include_ignored_ids: list[str], with_deleted: bool = False
    ) -> list[DirectoryDto]:
        with self.__database.get_db_session() as session:
            repository = session.get_repository(DirectoryCacheRepository)
            return [
                self._hydrate_to_directory_dto(cache)
                for cache in repository.get_all_include_ignored_ids(
                    with_deleted=with_deleted, include_ignored_ids=include_ignored_ids
                )
            ]
    ...
    def set_directory_cache(self, directory: DirectoryDto) -> None:
        ...
        with self.__database.get_db_session() as session:
            ...
            if cache is None:
                session.add(
                    DirectoryCache(
                        directory_id=directory.id, endpoint=directory.endpoint
                    )
                )
            else:
                ...
                cache.is_deleted = (
                    ...
                    if directory.is_deleted is not None
                    else cache.is_deleted
                )
            session.commit()
    ...
    def delete_directory_cache(self, directory_id: str) -> None:
        ...
        with self.__database.get_db_session() as session:
            ...
            if cache is None:
                ...
                raise ValueError(
                    f"Directory cache {directory_id} not found for deletion."
                )
